...
namespace PlatformerGameKit
{
    ...
    partial class AttackTransition : ITransitionGUI
    {
        ...
        public class Drawer : UnityEditorInternal.UnityEventDrawer
        {
            /************************************************************************************************************************/
            ...
            private static readonly MixedValueFieldGUI<int> DamageField = new MixedValueFieldGUI<int>(
                (index) => _Transition.Hits[index].Damage,
                (index, value) => _Transition.Hits[index].Damage = value,
                EditorGUI.IntField);

            private static readonly MixedValueFieldGUI<float> ForceField = new MixedValueFieldGUI<float>(
                (index) => _Transition.Hits[index].Force,
                (index, value) => _Transition.Hits[index].Force = value,
                EditorGUI.FloatField);

            private static readonly MixedValueFieldGUI<float> AngleField = new MixedValueFieldGUI<float>(
            ...
            private void DoGroupFields(ref Rect area)
            {
                ...
                using (ObjectPool.Disposable.AcquireContent(out var label, "Damage"))
                    DamageField.Draw(area, label, count, property);

                AnimancerGUI.NextVerticalArea(ref area);

                using (ObjectPool.Disposable.AcquireContent(out var label, "Force"))
                    ForceField.Draw(area, label, count, property);

                AnimancerGUI.NextVerticalArea(ref area);

                using (ObjectPool.Disposable.AcquireContent(out var label, "Angle"))
